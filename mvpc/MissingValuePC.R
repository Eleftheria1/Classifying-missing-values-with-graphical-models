...
mvpc <- function(suffStat, indepTest, corrMethod, alpha, p,
  ...
  skel_pre <- pcalg::skeleton(suffStat, indepTest, alpha,
    labels = labels,
  ...
  skel <- skeleton2(suffStat, corrMethod, alpha,
    skel_pre = skel_pre, labels = labels,
    method = skel.method, fixedGaps = fixedGaps, fixedEdges = fixedEdges,
    NAdelete = NAdelete, m.max = m.max, numCores = numCores,
    verbose = verbose
  )
  ...
  if (!conservative && !maj.rule) {
    ...
    udag2pdagRelaxed(pc.$sk,
      verbose = verbose, unfVect = pc.$unfTripl,
      solve.confl = solve.confl
    )
  }
}
...
skeleton2 <- function(suffStat, indepTest, alpha, labels, p, skel_pre,
  ...
  if (method == "stable.fast") {
    ...
    res <- .Call(
      "estimateSkeleton", G, suffStat, indepTestName,
      indepTest, alpha, fixedEdges, options
    )
    G <- res$amat
    sepset <- lapply(seq_p, function(i) {
      c(
        lapply(
          res$sepset[[i]],
    ...
    pval <- NULL
    if (!is.null(skel_pre@sepset)) {
      sepset <- skel_pre@sepset
    } else {
      sepset <- lapply(seq_p, function(.) vector("list", p))
    } # a list of lists [p x p]
  ...
  if (method == "stable.fast") {
    ...
    while (!done && any(G) && ord <= m.max) {
      ...
      for (i in 1:remEdges) {
        ...
        if (x != R_ind) {
          next
        } # we only focus on the missingness variable
        # ****** End adapted 2 ******
